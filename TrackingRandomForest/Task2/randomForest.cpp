...
void RandomForest::getNRand(std::unordered_set<int> * index, int max_number, int number_samples) {
...
void RandomForest::predict(cv::InputArray samples, Mat test_labels, cv::Mat * resp, cv::Ptr<cv::ml::TrainData> testData) {
	...
	for (int i = 0; i < nbDTrees; i++) {
		...
		std::cout << "Prediction error tree " << i << " : " << test_error << "%" << endl;
	}
